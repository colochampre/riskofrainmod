...
public class WispRenderer extends MobRenderer<WispEntity, WispModel<WispEntity>> {
  ...
  private static final ResourceLocation BEAM_LOCATION = new ResourceLocation(RoRmod.MODID, "textures/entity/wisp/hitscan_beam.png");
  ...
  @Override
  public boolean shouldRender(@NotNull WispEntity entity, @NotNull Frustum frustum, double p_114838_, double p_114839_, double p_114840_) {
  ...
  public void render(@NotNull WispEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight) {
    ...
    //renderHitScan(entity, partialTicks, poseStack, buffer);
    super.render(entity, entityYaw, partialTicks, poseStack, buffer, packedLight);
    poseStack.popPose();
  }
  /*
  private void renderHitScan(WispEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource) {
    LivingEntity target = entity.getActiveAttackTarget();
    if (target != null) {
      ...
      float f4 = (float) (vec32.length() + 1.5F);
  ...
  private static void vertex(VertexConsumer vertexConsumer, Matrix4f matrix4f, Matrix3f matrix3f, float f1, float f2, float f3, int i1, int i2, int i3, float f4, float f5) {
    vertexConsumer.vertex(matrix4f, f1, f2, f3).color(240, 212, 108, 255).uv(f4, f5).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(15728880).normal(matrix3f, 0.0F, 0.0F, 0.0F).endVertex();
  }
  */
  @NotNull
  @Override
